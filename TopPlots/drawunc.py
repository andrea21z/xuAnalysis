...
def DrawHisto(hname = 'H_NJets_ElMu_1btag', syst = 'ElecEff', systname = 'Electron Efficiency',  out = '',rebin = 1, Ymax='',ytit='Events', xtit = 'Jet Multiplicity'):
  ...
  histo.GetXaxis().SetRangeUser(5,120)
  ...
  hratio.GetXaxis().SetRangeUser(5,120)
  s = HistoComp(outpath = '/nfs/fanae/user/andreatf/PAFnanoAOD/temp%s_new/ver5/plots_syst/'%year, doNorm = False, doRatio = True)
  s.SetLumi(GetLumi(year))
  s.AddHisto(     histo,  'hist', '', 't#bar{t}')
  s.AddHisto(  histoUnc,  '', 'e2', systname)
  s.AddRatioHisto(hratio, 'e2')
  s.SetOutName(out)
  s.SetLegendPos(ncol = 1,x0 = 0.78, y0 = 0.65, x1 = 0.92, y1 = 0.91, size = 0.035)
